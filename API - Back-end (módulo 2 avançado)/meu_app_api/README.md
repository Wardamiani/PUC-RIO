...
O próximo passo será instalar as dependências/bibliotecas, descritas no arquivo, via docker:
```
Clicar com o botão direito no arquivo Dockerfile e selecionar a opção "Build Image...".
```
Por último, executar o comando:  
```
docker run pucriomain
```
```
Abra o [http://localhost:5000/#/](http://localhost:5000/#/) no navegador para verificar o status da API em execução.
```
